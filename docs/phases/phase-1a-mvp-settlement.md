...
| OA08 | Declined / not suitable | 🟩 variant 1 (not suitable, decided by the team, reason shown) 2026-09-26, API-tested; 🟩 variant 2 (lender answered without an offer → closed «ردّت جهتك الممولة» with the answer quoted as relayed) step 7, API-tested; ✅ variant 3 (withdrawn) step 5 | Rework + **Needs design (D-6)** | The reason comes from the team, or is the lender's response relayed by the team. The objection option arrives in step 8 |
| — | **Rahoon team workspace** (queue, review, completion, consent evidence, coordination log, updates, offer recording and verification, response relay) | ✅ 2026-09-26 for T01–T07 (`/team`, `/team/requests/[ref]`, `/team/verify`): queue tabs, review, assignment, identity check, completion request, consent evidence, coordination log, updates, messages and internal notes, not-eligible, documents; offer recording from the letter, verification by another member with checklist + step-up, response relay, continue, close with outcome; Playwright ✓ at 1440. T08 in step 8 | New + **Needs design (D-4)** «بانتظار اعتماد التصميم» | Built on the lender-shell layout with a «فريق رهون» sidebar; internal timers labelled «داخلي — لا يظهر للعميل» |
...
| D06 | Options | ✅ successor 2026-09-26: `/my/requests/[ref]/paths` — the four help paths, the ones being studied marked, no «متاح لك», no guarantee; the case-bound D06 stays for the lender mode | Rework — done for the MVP | Proposed D-1/D-5 text |
...
### Step 7: Approved offer and the individual's response ✅ 2026-09-26
- [x] The team records the lender's offer with its source document (T05): the lender letter is required (a request document of kind «خطاب الجهة الممولة»), path P1/P2/P3 with typed terms, «أثره عليك», lender reference and date, any validity as stated by the lender. A second member verifies it (T06, `/team/verify`): four-item match checklist, MFA step-up, **verifier ≠ recorder** enforced in code (refusal audited, even for the team lead who holds both permissions) and by a DB check constraint. A returned offer goes back to the recorder with a reason and never reaches the individual. Publishing supersedes the previous offer and (by default, D-5 proposal) shares the letter with the individual.
- [x] The individual sees D06 (paths explainer) and D07 (offer), then responds: D09 accept with an SMS-confirmed consent record (text snapshot + version), D08 question or suggestion (text required), or «لا يناسبني» (reason optional), which never triggers any action against them. Each response has a reference (`REQ-…-R1`) and time; submit is idempotent.
- [x] Response documented; the coordinator logs relaying it to the lender (T07: a `response_relay` coordination entry, visible to the individual as «نقلنا ردك…»). Only after relaying can the team continue coordinating (e.g. a new offer v2) or close with an outcome (offer accepted / declined / lender answered without an offer / other) and a summary shown to the individual. Execution of an accepted offer is Phase 1A-2.
...
- **Step 7, design pending:** T05–T07, D06, D07, D08 and D09 successors are built with the existing design system and the proposed D-5 text, «بانتظار اعتماد التصميم».
